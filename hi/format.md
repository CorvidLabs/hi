...
- **FILE-1**  As a reader, a hi file reads as an ordinary markdown document with no tool installed.
  - **FILE-1.a**  As a person writing intent, if the binary disappears tomorrow, nothing is lost, because hi keeps nothing of its own anywhere else.
...
- **FILE-2**  As a person writing intent, the machine-facing part of a file is its frontmatter, and nothing below it exists to serve the tool.
- **FILE-3**  As a person writing intent, when hi writes a line, it looks like something I would have typed by hand.
- **FILE-4**  As a person writing intent, hi never rewrites, reflows, or reformats prose that I wrote.
...
- **FILE-5**  As a person writing intent, one file covers one feature, and it can hold several id families.
- **FILE-6**  As a maintainer, one criterion is one line, however long the sentence runs, so I can grep it and diff it.
- **FILE-7**  As a person writing intent, hi understands my frontmatter whether I list the families inline or one per line, and when it adds a family it keeps the style I chose.
- **FILE-8**  As a person writing intent, if a write fails partway through, my file is left exactly as it was.
- **FILE-9**  As a person writing intent, a fenced code block in my prose is prose, so I can show an example of the format without it becoming real criteria.
- **FILE-10**  As a person writing intent, if my file uses Windows line endings, hi writes them back the same way.
- **FILE-11**  As a person writing intent, a byte-order mark from my editor does not make a valid file look broken.
...
- **FILE-13**  As a person writing intent, a file starts with my own words about why this feature exists, so the first thing anyone reads is the why and not the list.
- **FILE-14**  As a person writing intent, I can type a criterion into the file by hand, bullet or no bullet, bold or plain, and hi still reads it as one.
...
- **ID-1**  As a reader, an id never moves once written, so I can say it out loud a year later and still mean the same line.
  ...
  - **ID-1.c**  As a person writing intent, a zero-padded number is refused, because SEND-007 and SEND-7 must never be two names for one line.
- **ID-2**  As a person writing intent, I choose the id myself, because I am the one who has to say it.
...
- **ID-4**  As a person writing intent, if I write an id that breaks the alternation, hi tells me instead of quietly accepting it.
