...
| Export | Description |
|--------|-------------|
| `FORMAT_VERSION` | The one version of the file format this binary reads and writes. It is the number `hi:` carries in a file's frontmatter, and the number `hi export` reports as `hi`. Not the crate's version: releases change, the format does not have to (hi: FILE-25). |
...
| `Front` | The hand-parsed frontmatter: `hi:` version as a number and as the text the file wrote, declared `families`, `owner`, the inclusive line range of the `---` block, the inclusive line span of the `families` entry, and whether that entry was written as a YAML block list. |
| `unreadable_version` | `Front` method returning the declared `hi:` value when it is not a version this binary reads, exactly as the file wrote it, and `None` when the file is HI/1. An absent key and a key with no value are both HI/1; everything else — a later number, an earlier one, or text that is not a number — is returned for the caller to refuse by (hi: FILE-25). |
...
| `fence_marker` | The ``` or `~~~` run at the start of a trimmed line, with its length, or `None`. Public so every place hi reads markdown agrees on what a fence is: the parser treats a fenced block under `## Intent` as an example rather than structure, and `out::unwrap_soft_breaks` leaves the newlines inside one alone for the same reason (hi: FILE-9, ISSUE-7.b). |
...
| `fence_marker` | `fence_marker(trimmed: &str) -> Option<(char, usize)>` | For `` ` `` then `~`, counts the leading run of that character and returns it with its length once the run is three or more; otherwise `None`. The caller owns the open/close state machine: a fence closes only on the same marker character with a run at least as long as the one that opened it. |
...
| 2026-09-18 | Claude | `FORMAT_VERSION` and `Front::unreadable_version` added, at 24 exports. The `hi:` key was parsed, stored, and read by nothing outside this module: a file declaring `hi: 2` was parsed, checked and appended to as HI/1, which meant the number could never be frozen and an HI/2 could never ship, because every binary already released would edit an HI/2 file believing it understood it. `Front` now also keeps `version_text`, the value exactly as the file wrote it, because a value that is not a number parses to `None` and so does an absent key. This module only reports; `workspace::load` refuses. Added REQ-doc-021 (hi: FILE-25, DECISIONS.md §38). |
