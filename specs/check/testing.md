...
| `src/check.rs` (`prose_outside_a_section_is_not_a_stray_criterion`) | Unit | REQ-check-011. The same shape with ordinary prose below the heading is clean. Only an id-shaped first token is a stray. |
...
| `tests/cli.rs` (`outside_a_repository_it_says_so_rather_than_guessing`) | Integration | REQ-check-009. `hi check` with no workspace exits 1 from `Workspace::find` with `no hi/ directory found`, before `run` is ever called. That exit is not a `Report`. |
...
| REQ-check-011 (the `## Intent` exception) | No test anywhere puts an unfenced id-shaped line at column 0 under `## Intent` and asserts it is neither a criterion nor a stray. `doc::tests::a_fenced_block_in_intent_is_not_parsed_as_criteria` covers only the fenced case, where the fence would have been enough on its own. |
...
- [ ] Write an unfenced `SEND-9  ...` at column 0 under `## Intent` and confirm `hi check` reports
...
| Id-shaped token with a bad family (`Send-1`), no hyphen (`SEND1`), or nothing after it (`SEND-`) | `doc` never treats the line as a criterion, because `looks_like_id` requires a valid family and a non-empty remainder, so it is prose and produces no problem. `IdError::BadFamily`, `MissingHyphen` and `NoLevels` are therefore unreachable from `check`. |
| Unparseable ids that do reach `check` | `SEND-1.` (`EmptyLevel`), `SE-ND-1` (`BadLevel`), `SEND-007` (`PaddedLevel`), and `SEND-1.a.b` (`Alternation`). These four are the only `IdError` variants a criterion line can carry, and each appends its own `Display` text to the message. |
...
| An id-shaped line at column 0 under a `## ` heading `doc` does not recognize (`## Notes`) | `StrayCriterion`. Only `## Criteria` and `## Retired` open a section, so any other `## ` heading leaves the same gap a `# ` heading does. |
| An id-shaped line at column 0 between the frontmatter and the first heading | `StrayCriterion`. No section has been opened yet, so the line is stranded in the same way. |
| An id-shaped line at column 0 under `## Intent` | No criterion, no stray, no problem. `Doc::parse_body` pushes every line of an intent section into the intent prose before it reaches the stray branch, so the line stays prose and reappears in `view` and `export`. The heading name is matched case-insensitively. |
...
| Continuation lines under a criterion | Never checked; only the id-bearing line is a criterion, and its `line_no()` is the reported location. |
