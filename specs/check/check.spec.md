...
| `new` | `Note` constructor taking a kind and anything that converts into a `String`. |
...
| `Report` | Everything one check run found: file/criteria/retired counts, families, problems, and the notes as a list, which are never problems and never reach the exit code. |
...
| `Report` | `struct` with public fields `notes: Vec<Note>`, `files: usize`, `criteria: usize` (active only), `retired: usize`, `families: Vec<String>` (sorted), and `problems: Vec<Problem>` (sorted by file then line). Derives `Debug, Clone, Serialize`. `notes` is deliberately not a `Vec<Problem>` and `NoteKind` is deliberately not a seventh `Kind`: `Report::ok` does not read the notes, so nothing in them can move the exit code (hi: CHECK-1). It replaced a single `Option<String>` holding every note joined with `\n      `, which put the terminal's indentation inside the data and left `--json` unable to carry the notes apart (hi: CHECK-6). |
...
| `serde` | `Serialize` derive on `Problem`, `Note` and `Report`, plus hand-written `Serialize` impls on `Kind` and `NoteKind` that write `self.code()`, for `hi check --json`. |
...
| `out` | `out::index_note`, read-only, for the note about the generated feature list, and nothing else in the module. It returns a `check::Note` so the note's code is decided beside the other three rather than in a second place (hi: INDEX-4.b, CHECK-6). |
