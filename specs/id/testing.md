...
| `src/promise.rs` | Property | REQ-id-012. Generated files hi did not write, random capture / retire / hand-edit sequences. After every step: saved ids are readable in the named section, unnamed shapes are unchanged, no id is assigned twice, `hi check` exiting 0 implies all three |
| `tests/promise.rs` | Integration, concurrent | REQ-id-012. Concurrent captures of distinct ids and of the same id, and a concurrent retire + capture, all against a file hi did not write |
