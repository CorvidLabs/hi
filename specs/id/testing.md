...
Ten inline `#[test]` functions live in `src/id.rs` under `mod tests`. Run them with
`cargo test --bin hi id::tests`, or the whole gate with `fledge run test`. The end-to-end refusals
live in `tests/cli.rs` and run with `cargo test --test cli`.
...
| `src/id.rs` (`id::tests`) | Unit | `recognises_id_shaped_tokens` (REQ-id-009): `SEND-1`, `SEND-1.a`, and the malformed `SEND-a.b` are id-shaped; `I`, `well-formed`, and `Given` are not |
...
- [ ] Add a line reading `SEND-a.b something` to a scratch hi file and run `hi check`: the problem
      is reported at its file and line rather than skipped as prose (REQ-id-009).
...
| A padded id already written in a file (a line that starts `SEND-007`) | `looks_like_id` is `true` and `Id::parse` fails, so `doc` stores the `IdError` and `check` reports `unparseable-id` at that line rather than reading a different criterion. Not directly asserted, a gap |
...
| `IdError` `Display` wording for any variant | Interpolated into `hi check` and capture output. Only `Alternation`'s `expected` string is pinned by a unit test, through equality rather than through its rendered sentence; `PaddedLevel`'s sentence is pinned only by the substring "leading zero" in `a_zero_padded_id_is_refused_rather_than_silently_renamed`. That leaves a gap |
