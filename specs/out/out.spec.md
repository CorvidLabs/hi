...
| `export` | Build the agent payload as pretty-printed JSON for a family, a file named by its stem, its file name, or any path ending in `hi/<stem>.md`, one criterion id with its cases and the criteria above it, or the whole repository. |
...
| `agent_instructions` | The text of `hi/AGENTS.md`: the habit an agent follows before building, and nothing else. Shared with `capture`, which writes it on the first capture so an agent finds it without being told (hi: HABIT-1, HABIT-2, HABIT-3). It carries no id grammar, no file format and no list of existing families, because the file is written once and never rewritten, so anything hi could change underneath it would be wrong later with nothing to notice (DECISIONS.md §27). It says two things beyond the habit, and both were admitted on the same test — not "is it one more sentence" but "can it ever become false". Prose here is one line per paragraph, which is how markdown reads a newline rather than anything about hi's format (DECISIONS.md §29, hi: FILE-21). And after a merge that touched `hi/`, run `hi check`, because two branches can each choose the same id and git merges both without saying anything; that is a habit, which is the category §27 said this file carries, and it rests only on `duplicate-id` being one of the structural problems HI-1.md freezes (DECISIONS.md §37, §38, hi: HABIT-5). The text is itself one line per paragraph (hi: FILE-21.a). |
...
| `index_note` | Read-only. What to say when the generated block in `INTENT.md` no longer matches the workspace (`index-behind`), or when a broken marker pair means nothing can refresh it (`index-markers`). Returns a `check::Note`, so the codes a script matches on live beside check's other two rather than in a second place. Used by `check` as a `note:`, never as a problem and never as an exit code (hi: INDEX-4.b, CHECK-1, CHECK-6). |
| `AgentTemplate` | What `hi/AGENTS.md` currently is, relative to the templates hi has shipped: `Current`, `Prior`, or `Other`. |
| `classify_agent_file` | Byte identity after folding a BOM and CRLF, against `agent_instructions` and the templates in `src/seed/`. |
| `write_with_endings` | Write text using the line endings a previous body was stored with. |
...
| `AgentTemplate` | `enum` with three variants: `Current` (byte-identical to `agent_instructions` after folding), `Prior` (byte-identical to a template in `src/seed/`), `Other` (the person edited it). Derives `Debug, Clone, Copy, PartialEq, Eq`. |
| The JSON payload types | `Export`, `ExportFile` and `ExportCriterion` are private; the payload is a contract expressed as JSON, not as Rust types, so nothing outside this module constructs them. `Export` carries two version fields: `hi`, the format the files are in (`doc::FORMAT_VERSION`), and `export`, the shape of the payload itself (the private `ENVELOPE_VERSION`). They are both `1` and they are free to move apart, which is the whole reason they are two fields (hi: EXPORT-6). |
...
| `index_note` | `fn index_note(workspace: &Workspace) -> Option<crate::check::Note>` | Reads `INTENT.md`. With both marker lines present, compares the bytes between them to `INDEX_OPEN` + `index_block` + `INDEX_CLOSE` and returns `<path>'s feature list is behind what is captured. Run \`hi index\`` when they differ. With an opening marker line that `index_span` could not close, returns `<path> has an opening <!-- hi:index --> with no matching <!-- /hi:index -->, so nothing can refresh its feature list`. Returns `None` for a missing or unreadable `INTENT.md`, for one with no marker line at all, and for a block that already matches. Writes nothing. |
| `classify_agent_file` | `fn classify_agent_file(raw: &str) -> AgentTemplate` | Fold a BOM and CRLF, then compare to `agent_instructions` and each template in `src/seed/`. Byte identity is the whole of the recognition (hi: HABIT-6.a, HABIT-6.b). |
| `write_with_endings` | `fn write_with_endings(path: &Path, text: &str, like: &str) -> io::Result<()>` | If `like` contains CRLF, write `text` with CRLF; otherwise write it as-is. Used by `seed_agent_files` so a rewrite does not flip a Windows file to LF. |
...
### Scenario: Export scoped to one id

- **Given** `hi/chat.md` holding `SEND-1`, `SEND-1.a`, `SEND-1.a.1`, `SEND-1.b` and `SEND-2`
- **When** `export` is called with scope `SEND-1.a.1`
- **Then** `scope` is `"SEND-1.a.1"`, `files` holds `hi/chat.md` with its intent, and `criteria`
  holds `SEND-1`, `SEND-1.a` and `SEND-1.a.1`, so every `parent` names an entry in the payload
- **And** the top-level and per-file keys are the same as a family export's (hi: EXPORT-3)
...
| `export` is given a scope that selects no file | `nothing matches '<scope>'. Give a family like SEND, a file like chat, an id like SEND-1, or nothing at all for the whole repository` |
| `export` is given a family that frontmatter declares but no criterion uses | The same `nothing matches '<scope>'. Give a family like SEND, a file like chat, an id like SEND-1, or nothing at all for the whole repository`. File selection needs a criterion in `doc.all()`, so a declared-but-unused family reaches no file even though `Workspace::families` lists it. The message no longer claims the family does not exist, but it still tells the person to give a family when they gave one, and never says the family is declared and empty |
...
| 2026-09-18 | Claude | Two changes, no export count change at 12. `Export` carries `export` beside `hi`: `hi` stays the *file format*'s version and `export` is the shape of this payload, because one field could never later mean both and splitting them is impossible once anything depends on the shape (hi: EXPORT-6, new REQ-out-020). `index_note` returns a `check::Note` rather than a `String`, so its two codes are decided beside check's other two (hi: CHECK-6). `agent_instructions` gains one sentence: after a merge that touched `hi/`, run `hi check`. That is the second narrowing of DECISIONS.md §27, admitted on the same test as the first — it is a habit rather than a fact about a format that is not frozen, and the only thing it rests on is `duplicate-id` being one of the six (hi: HABIT-5, DECISIONS.md §37, §38). Amended REQ-out-016 and REQ-out-018. |
| 2026-09-18 | Claude | `AgentTemplate`, `classify_agent_file` and `write_with_endings` added, with REQ-out-021, so `hi seed` can tell a template hi shipped from a file a person edited without capture ever rewriting one (hi: HABIT-6, DECISIONS.md §39). |
| 2026-09-22 | Claude | `export` accepts one criterion id as a scope and returns it with its cases, the criteria above it, and its file's intent, in the unchanged envelope (hi: EXPORT-7, EXPORT-7.a, new REQ-out-022). The private `in_slice` decides membership; the refusal names an id as the fourth thing a scope can be. `agent_instructions` gains one sentence naming the habit, and the 0.8 text joins `src/seed/` as a known template (DECISIONS.md §40). An id scope takes precedence over a frontmatter-declared family of the same name, because declared families are not validated; a retired case still brings the live criteria above it; an id-shaped scope that does not parse is refused with the parse reason. One `in_scope` filter now decides both file selection and what is kept. Public API is unchanged in count. |
