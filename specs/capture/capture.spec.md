...
9. A missing `hi/` directory is created on demand, with `fs::create_dir_all`, and there is no init
   ...
   (DECISIONS.md §31). `capture::capture` still creates it if it is missing when it comes to save,
...
| 2026-09-17 | Claude | Restated invariant 9: `hi/` is now created by `lock::acquire`, before the lock rather than during the write, because the lock file lives inside the directory and the first capture in a repository was therefore never locked (hi: FILE-19, DECISIONS.md §31). `capture` keeps its own `create_dir_all` for a direct caller. Noted that a guard which created the directory removes it again while it is empty, so `CAPTURE-5` still holds for a refusal in a repository that had no `hi/`. |
