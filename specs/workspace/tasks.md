...
- [ ] Extend the inline `#[cfg(test)]` module in `src/workspace.rs`, which today holds only `relative_paths_always_use_forward_slashes`, to cover `Workspace::find`: the ancestor walk from a nested directory, the nearest-recognized-`hi/`-wins case across two nested workspaces, and the not-found error message byte for byte (REQ-workspace-001). The cli tests cover the walk end to end but nothing exercises `find` in isolation, and only the `not a repository` fragment of the message is pinned anywhere.
- [ ] Finish covering the `.git` boundary. `tests/cli.rs::a_repository_is_a_boundary_for_discovery` proves the stop with `check`; what is still unproven is a *capture* at a `.git` root with no `hi/`, which is what creates the directory and the first file, and a `.git` file rather than a directory (REQ-workspace-001, hi: CAPTURE-10, CAPTURE-1.a).
...
- [ ] Cover `rel` on a path outside `root`, the one branch of the function the new inline test does not reach. Decide first whether the doubled leading separator it currently returns for an absolute path is acceptable, since no caller in hi produces one (REQ-workspace-008).
...
- `Workspace::find` has no unit test. `tests/cli.rs` covers the walk, the recognition test, the `.git` boundary and the not-found failure through the real binary, but nothing calls `find` directly. The cli `Repo` helper never creates a `.git`; `a_repository_is_a_boundary_for_discovery` makes one by hand and is the only test that does.
- Only the fragment `not a repository` is asserted anywhere. The rest of the message, `and no hi/ directory was found above it. hi anchors to a repository, so run it inside one`, could change without failing a test.
...
- `rel`'s outside-`root` fallback is unexercised. `relative_paths_always_use_forward_slashes` covers only paths under `root`.
